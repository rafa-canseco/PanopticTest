...
interface LeaderboardProps {
  summaries: UserPointsSummary[];
  selectedUserId: string;
  onSelect: (userId: string) => void;
}

export function Leaderboard({ summaries, selectedUserId, onSelect }: LeaderboardProps) {
  const ranked = [...summaries].sort((a, b) => b.totalPoints - a.totalPoints);

  return (
    <div className="rounded-lg border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
      <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
        <h2 className="text-sm font-semibold text-zinc-950 dark:text-zinc-50">Leaderboard</h2>
        <span className="text-xs text-zinc-500">click a row to inspect</span>
      </div>
          ...
          <tbody>
            {ranked.map((s, i) => {
              ...
              return (
                <tr
                  key={s.user.id}
                  onClick={() => onSelect(s.user.id)}
                  className={
                    "cursor-pointer border-b border-zinc-100 transition-colors last:border-b-0 dark:border-zinc-800 " +
                    (selected
